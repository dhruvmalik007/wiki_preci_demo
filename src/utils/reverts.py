...
class Reverts:
    ...
    def store_results(self,page_title):
        """
        gets the csv file result downloaded for later analysis
        
        
        """
        
        df = self.get_latest_revision_date(page_title)
        df.to_csv(page_title +'.csv')
    
    
    
    def get_revisions_with_tags_and_reverts(self, page_title, showcase_records) -> DataFrame:
        """
        ...
        Args:
            ...
            showcase_index: only shows the first n titles 
        ...
        title = self.get_page_name(page_title)
        while continue_flag:
            # Construct API URL
            api_url = f"https://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles={title}&rvlimit=max&rvprop=tags|user|content&format=json{continue_params}"
            ...
            if 'continue' in data:
                continue_params = '&rvcontinue=' + \
                    data['continue']['rvcontinue']
            else:
                continue_flag = False
        st.info("test info got from the ")
        # Return revisions as a pandas DataFrame for cleaner tabular display
        df = pd.DataFrame(revisions_with_tags_reverts)
        df_last_reverts = df.tail(showcase_records)
        df_last_reverts.to_csv(page_title + '_' + (time.time_ns) + '.csv')
        return df_last_reverts
    ...
    def is_revert(self, current_content, previous_content) -> bool:
        ...
        try:
            ...
            revert_keywords = ["revert", "rv",
                               "undo", "rvv", "reverted", "revision"]
    ...
    def count_revisions(self, topic):
        ...
        while continue_flag:
            ...
            page_id = list(data["query"]["pages"].keys())[0]
            revisions_count += len(data["query"]
    ...
    def get_page_name(self, search_term):
        ...
        try:
            ...
            data = response.json()
            page_name = data['query']['search'][0]['title']
        except Exception as e:
            print(e)
            page_name = None
        return page_name
    ...
    def get_revision_history(self, page_name):
        title = self.get_page_name(page_name)
        params = {
            "action": "query",
            "format": "json",
            "prop": "revisions",
        ...
        try:
            ...
            if page_name in data["query"]["pages"]:
                page_id = list(data["query"]["pages"].keys())[0]
                st.info("getting the page id" + str(page_id))
                revisions = data["query"]["pages"][page_id]["revisions"]
            ...
            print("in get revision history" + str(e))

        st.info(revision_data)

    def getting_average_revert_parameter():
        pass
...
# api.php?action=query&prop=revisions&titles=API:Geosearch&rvlimit=5&rvslots=main&rvprop=timestamp|user|comment&rvdir=newer&rvstart=2018-07-01T00:00:00Z&
    ...
    def get_latest_revision_date(self, article_title):
        ...
        newest_revision_date = datetime.datetime.strptime(
            newest_revision_timestamp, "%Y-%m-%dT%H:%M:%SZ")
...
class CloudFunctions():
    ...
    #     page_key = self.datastore_client.key(
    #         'WikipediaPageHistory', pageHistory.title)
